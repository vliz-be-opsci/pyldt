...
from pysubyt import Generator, Settings, Source, Sink
...
class CountingSink(Sink):
    """ Just verifies that added parts are incrementing up to the expected size
    """
    def __init__(self, size):
        self._size = size
        self._count = 0
    ...
    def close(self):
        assert self._count == self._size, "sink did not receive expected number of added parts"
...
class SimplePassGenerator(Generator):
    ...
    def __init__(self, testname, size):
        assert size > 2, "test only makes sense to check beyond first/last cases"
        self._name = testname
        self._size = size
        self._count = 0
