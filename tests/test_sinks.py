        ...
        items = [
            {
                "id": i,
                "key": "%04d" % i,
                "data": "data-%d-%s" % (i, ''.join(random.choices(string.ascii_uppercase + string.digits, k=20)))
            } for i in range(count)
        ]

        with tempfile.TemporaryDirectory(dir=temp) as temp_folder:
            sfs = SinkFactory.make_sink(os.path.join(temp_folder, "all.out"))
            self.assertTrue(isinstance(sfs, SingleFileSink), "expected single file sink")

            pfs = SinkFactory.make_sink(os.path.join(temp_folder, "item-{key}.out"))
            self.assertTrue(isinstance(pfs, PatternedFileSink), "expected Patterned file sink")
